import 'dart:async' show FutureOr;

import 'package:bloc/bloc.dart' as bloc show BlocBase, Bloc, Cubit;
import 'package:bloc_test/bloc_test.dart' as bloc_test;
import 'package:flutter_test/flutter_test.dart';
import 'package:fpdart/fpdart.dart' show Either, TaskEither;
import 'package:meta/meta.dart' show isTest, protected;
import 'package:mocktail/mocktail.dart' show registerFallbackValue, reset;

import '../export.dart' show MviCubit, TcaBloc, TcaEffect, TcaReducer;

part 'test_suite_bloc.dart';
part 'test_suite_cubit.dart';

/// **Bloc Arch Flow Pattern**
///
/// An approach provided by the `bloc_arch_flow` package. It offers a predictable state flow
/// and structured business logic by adopting either MVI (Model-View-Intent) or
/// TCA (The Composable Architecture) architectural principles alongside Bloc.
///
/// The [BlocBaseTestSuite] class is an abstract base class for testing a Bloc or Cubit
/// that follows the **Bloc Arch Flow pattern**. Inherit from this class to define the setup
/// for the specific Bloc/Cubit you want to test.
///
/// - [E]: The mock type for the Bloc or Cubit's Environment (dependencies) (e.g., `MockCounterEnvironment`).
/// - [B]: The type of the Bloc or Cubit to be tested (e.g., `CounterMviBloc`, `CounterTcaBloc`).
/// - [S]: The state type of the Bloc or Cubit (e.g., `CounterState`).
/// ---
/// **Bloc Arch Flow 패턴**
///
/// 이 `bloc_arch_flow` 패키지에서 제공하는 접근 방식으로, Bloc과 함께 MVI (Model-View-Intent)
/// 또는 TCA (The Composable Architecture) 아키텍처 원칙 중 하나를 선택하여,
/// 예측 가능한 상태 흐름과 구조화된 비즈니스 로직을 제공하는 접근 방식입니다.
///
/// [BlocBaseTestSuite] 클래스는 **Bloc Arch Flow 패턴**을 따르는 Bloc 또는 Cubit을 테스트하기 위한 추상 기본 클래스입니다.
/// [BlocBaseTestSuite] 클래스를 상속받아 테스트할 특정 Bloc/Cubit에 대한 설정을 정의합니다.
///
/// - [E]: Bloc 또는 Cubit의 Environment(의존성) Mock 타입 (예: `MockCounterEnvironment`).
/// - [B]: 테스트할 Bloc 또는 Cubit의 타입 (예: `CounterMviBloc`, `CounterTcaBloc`).
/// - [S]: Bloc 또는 Cubit의 상태 타입 (예: `CounterState`).
abstract interface class BlocBaseTestSuite<B extends bloc.BlocBase<S>, S, E> {
  /// The Mock Environment instance that will be initialized before each test case.
  ///
  /// ---
  /// 테스트 전 각 테스트 케이스에 대해 초기화될 Mock Environment 인스턴스입니다.
  late E mockEnvironment; // 의존성

  late B mockBaseBloc; // bloc or cubit

  late S initState; // 초기 상태

  /// Creates and returns the Mock Environment instance.
  /// This must be implemented by the user for each test suite.
  ///
  /// ---
  /// Mock Environment 인스턴스를 생성하고 반환합니다.
  /// 사용자가 각 테스트 스위트에 맞게 구현해야 합니다.
  E buildMockEnvironment();

  /// Returns the initial state of the Bloc or Cubit to be tested.
  /// This may be needed for `bloc_test`'s [registerFallbackValue].
  /// ---
  /// 테스트할 Bloc 또는 Cubit의 초기 상태를 반환합니다.
  /// `bloc_test`의 [registerFallbackValue]를 위해 필요할 수 있습니다.
  ///
  S buildInitialState();

  /// Builds and returns the Bloc or Cubit instance to be tested.
  /// This method is used in the `build` callback of `blocTest`.
  ///
  /// ---
  /// 테스트할 Bloc 또는 Cubit 인스턴스를 빌드하여 반환합니다.
  /// 이 메서드는 `blocTest`의 `build` 콜백에서 사용됩니다.
  B buildMockBaseBloc();

  /// Initializes this test group.
  /// Call this at the beginning of a `group` function to set up the `setUp` and `tearDown` logic.
  ///
  /// ---
  /// 이 테스트 그룹을 초기화합니다.
  /// `group` 함수의 시작 부분에서 호출하여 `setUp` 및 `tearDown` 로직을 설정합니다.
  void initTestSuite() {
    setUp(() {
      // Mocktail의 `any()` 매처가 예상치 못한 타입의 인자를 받을 때를 대비하여 fallback을 등록합니다.
      // 특히, 상태 객체나 이벤트/액션 객체를 Mock 함수에 전달할 때 유용합니다.
      mockEnvironment = buildMockEnvironment();
      mockBaseBloc = buildMockBaseBloc();
      initState = buildInitialState();

      // Mocktail의 `any()` 매처가 객체를 반환해야 하는 경우,
      // 해당 타입에 대한 fallback을 등록합니다.
      registerFallbackValue(initState);
    });

    tearDown(() {
      // 각 테스트 후 Mock 상태를 초기화하여 테스트 간 독립성을 보장합니다.
      reset(mockEnvironment);
    });
  }

  /// A wrapped `blocTest` function for a Bloc or Cubit that follows the Bloc Arch Flow pattern.
  /// This function provides `build: () => buildMockBaseBloc()` as a default, which simplifies test code.
  ///
  /// You can explicitly provide a `build` callback to perform additional setup, such as
  /// mocking configurations, before the Bloc/Cubit under test is built.
  ///
  /// - [description] A description of the test case.
  /// - [build] (Optional) A function that returns the Bloc/Cubit instance to be tested.
  /// By default, calls `buildMockBaseBloc()`.
  /// - [setUp] (Optional) Additional setup before each test case.
  /// - [seed] (Optional) The initial state of the Bloc/Cubit.
  /// - [act] (Optional) A function that performs an action on the Bloc/Cubit.
  /// - [wait] (Optional) The duration to wait between `act` and `expect`.
  /// - [skip] (Optional) The number of tests to skip.
  /// - [expect] (Optional) A list of expected state changes or a Matcher.
  /// - [verify] (Optional) Additional verification logic.
  /// - [errors] (Optional) A list of expected errors or a Matcher.
  /// - [tearDown] (Optional) Cleanup after each test case.
  /// - [tags] (Optional) Test tags.
  ///
  /// ---
  /// Bloc Arch Flow 패턴을 따르는 Bloc 또는 Cubit을 위한 래핑된 `blocTest` 함수입니다.
  /// 이 함수는 `build: () => buildMockBaseBloc()`를 기본값으로 제공하여 테스트 코드의 간결성을 높입니다.
  ///
  /// `build` 콜백을 명시적으로 제공하여 테스트 대상 Bloc/Cubit이 빌드되기 전에
  /// Mocking 설정과 같은 추가적인 준비 작업을 수행할 수 있습니다.
  ///
  /// - [description] 테스트 케이스에 대한 설명.
  /// - [build] (선택 사항) 테스트할 Bloc/Cubit 인스턴스를 반환하는 함수.
  ///           기본적으로 `buildMockBaseBloc()`을 호출합니다.
  /// - [setUp] (선택 사항) 각 테스트 케이스 실행 전 추가 설정.
  /// - [seed] (선택 사항) Bloc/Cubit의 초기 상태.
  /// - [act] (선택 사항) Bloc/Cubit에 액션을 실행하는 함수.
  /// - [wait] (선택 사항) `act`와 `expect` 사이에 기다릴 시간.
  /// - [skip] (선택 사항) 건너뛸 테스트 수.
  /// - [expect] (선택 사항) 예상되는 상태 변화 목록 또는 Matcher.
  /// - [verify] (선택 사항) 추가적인 검증 로직.
  /// - [errors] (선택 사항) 예상되는 오류 목록 또는 Matcher.
  /// - [tearDown] (선택 사항) 각 테스트 케이스 실행 후 정리 작업.
  /// - [tags] (선택 사항) 테스트 태그.
  @isTest
  void blocTest(
    String description, {
    B Function()? build,
    FutureOr<void> Function()? setUp,
    S Function()? seed,
    dynamic Function(B bloc)? act,
    Duration? wait,
    int skip = 0,
    dynamic Function()? expect,
    dynamic Function(B bloc)? verify,
    dynamic Function()? errors,
    FutureOr<void> Function()? tearDown,
    dynamic tags,
  }) {
    bloc_test.blocTest<B, S>(
      description,
      build: build ?? () => buildMockBaseBloc(),
      setUp: setUp,
      seed: seed,
      act: act,
      wait: wait,
      skip: skip,
      expect: expect,
      verify: verify,
      errors: errors,
      tearDown: tearDown,
      tags: tags,
    );
  }
}

/// A mixin that provides utility methods for creating `Either` and `TaskEither` instances
/// for testing purposes.
///
/// This mixin simplifies the creation of success (right) and failure (left)
/// scenarios, making test code more readable and concise.
///
/// 테스트 목적으로 `Either` 및 `TaskEither` 인스턴스를 생성하기 위한 유틸리티 메서드를
/// 제공하는 믹스인입니다.
///
/// 이 믹스인은 테스트 코드의 가독성과 간결성을 높이기 위해 성공(right) 및 실패(left)
/// 시나리오를 쉽게 생성할 수 있도록 돕습니다.
mixin TestSuiteUtilityMixin {
  /// Returns a `TaskEither` instance representing a successful result (Right).
  ///
  /// This is useful for mocking asynchronous operations that are expected to succeed.
  ///
  /// ---
  /// 성공적인 결과(Right)를 나타내는 `TaskEither` 인스턴스를 반환합니다.
  ///
  /// 성공이 예상되는 비동기 작업을 모의(mock)할 때 유용합니다.
  TaskEither<L, R> whenSuccessTask<L, R>(R right) => TaskEither.right(right);

  /// Returns a `TaskEither` instance representing a failed result (Left).
  ///
  /// Use this to mock asynchronous operations that are expected to fail.
  ///
  /// ---
  /// 실패한 결과(Left)를 나타내는 `TaskEither` 인스턴스를 반환합니다.
  ///
  /// 실패가 예상되는 비동기 작업을 모의(mock)하는 데 사용합니다.
  TaskEither<L, R> whenFailureTask<L, R>(L left) => TaskEither.left(left);

  /// Returns an `Either` instance representing a successful result (Right).
  ///
  /// This method simplifies the creation of synchronous success scenarios in tests.
  ///
  /// ---
  /// 성공적인 결과(Right)를 나타내는 `Either` 인스턴스를 반환합니다.
  ///
  /// 동기적인 성공 시나리오를 테스트에서 쉽게 생성할 수 있도록 합니다.
  Either<L, R> whenSuccess<L, R>(R right) => Either.right(right);

  /// Returns an `Either` instance representing a failed result (Left).
  ///
  /// This is useful for mocking synchronous operations that are expected to fail.
  ///
  /// ---
  /// 실패한 결과(Left)를 나타내는 `Either` 인스턴스를 반환합니다.
  ///
  /// 실패가 예상되는 동기적인 작업을 모의(mock)하는 데 유용합니다.
  Either<L, R> whenFailure<L, R>(L left) => Either.left(left);
}